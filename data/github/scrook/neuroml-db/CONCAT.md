...
Output: Two files: batch_insert_statements.sql and cp_statements.sh, which contain the SQL statements + shell copy statements needed to insert the models into the database and copy their .nml files to the webserver, respectively.## Neuro Science Gateway Quick start guide:
